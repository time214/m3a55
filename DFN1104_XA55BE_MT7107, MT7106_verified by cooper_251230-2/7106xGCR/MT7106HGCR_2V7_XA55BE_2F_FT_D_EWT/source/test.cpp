...
DUT_API int Vout(short funcindex, LPCTSTR funclabel)
{
	...
	///////////////////////// Vout1 /////////////////////////				3.9v,10mA, vout1, offset 0mv
	...
	/////////////////////////// 150mA /////////////////////////		3.9v,150mA, v4,  offset 0mv
	...
	/////////////////////////// Vout2 /////////////////////////			3.9v,300mA, vout2, offset -4mv
	...
	delay_ms(5);
	///////////////////////// Vout3 /////////////////////////			5.5v,10mA, vout3, offset -4mv
	...
	vout.MeasureVI(MV, 10, 20);	// 251028
	for (i=0;i<SITENUM;i++)
	{
		v3[i]=vout.GetMeasResult(i)-0.000f;	// 251028, -0.004v
...
DUT_API int Ven(short funcindex, LPCTSTR funclabel)
{
	...
	for(k=1.4f; k>0.85f; k=k-0.01f )  
	{
		...
		delay_us(200);
		vin.MeasureVI(MI,100,20);// 251028
		for(i=0; i<SITENUM; i++ )  
		{ 
...
DUT_API int UVLO(short funcindex, LPCTSTR funclabel)
{
	...
	////vin.Set(FV, 2.40f, PVI10A_10V, PVI10A_100MA,RELAY_ON);
	vin.Set(FV, 3.3f, PVI10A_10V, PVI10A_100MA,RELAY_ON);
	en.Set(FV, 2.0f, DVI400_5V, DVI400_400UA, RELAY_ON);
	vout.Set(FI, -0.5e-3, PVI10A_5V, PVI10A_100MA,RELAY_ON);
	delay_ms(1);
	...
	for (k=2.3f; k>1.7f; k= k-0.01f)
	{
		...
		{ 
			...
			if(adresult[i+12]<20.0e-6f && check[i]<0.5f)		// 251029
			{
				adresult[i+16]=k;
				check[i]=1.0f;
			}
...
DUT_API int OVP(short funcindex, LPCTSTR funclabel)
{
	...
	for (k=6.5f; k>5.5f; k=k-0.01f)
	{
		...
		{ 
            ...
            ////if(adresult[i+12]>90.0e-6f && check[i]<0.5f)	// 251029
			{
				adresult[i+16]=k;
				check[i]=1.0f;
			}
...
DUT_API int Dropout(short funcindex, LPCTSTR funclabel)
{
	...
	for ( i=0; i<SITENUM; i++)
	{
		for (vx=3.9f; vx>(3.0f*0.9f); vx=vx-0.01f)
	    {
