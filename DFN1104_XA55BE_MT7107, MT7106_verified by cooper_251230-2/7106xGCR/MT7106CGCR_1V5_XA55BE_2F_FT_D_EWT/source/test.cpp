...
DUT_API int Vout(short funcindex, LPCTSTR funclabel)
{
	...
	vout.Set(FI, -5.0e-3, PVI10A_5V, PVI10A_1A,RELAY_ON);		// 251029
	delay_ms(5);
	vout.Set(FI, -1.0e-3, PVI10A_5V, PVI10A_1A,RELAY_ON);
	delay_ms(1);
	...
	for (i=0;i<SITENUM;i++)
	{
		...
		v1[i]+=-float(0.01);	// 2512230-2   for 7106cgcr
	...
	for (i=0;i<SITENUM;i++)
	{
		v3[i]=vout.GetMeasResult(i)-0.000f;		// 251028
...
DUT_API int UVLO(short funcindex, LPCTSTR funclabel)
{
	...
	////vin.Set(FV, 2.6f, PVI10A_10V, PVI10A_100MA,RELAY_ON);
	vin.Set(FV, 1.8f, PVI10A_10V, PVI10A_100MA,RELAY_ON);
	en.Set(FV, 2.0f, DVI400_5V, DVI400_400UA, RELAY_ON);
	vout.Set(FI, -0.5e-3, PVI10A_5V, PVI10A_100MA,RELAY_ON);
	delay_ms(1);
