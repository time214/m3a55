...
DUT_API int Iq(short funcindex, LPCTSTR funclabel)
{
    ...
    ////////////////////////// Isd //////////////////////////
	en.Set(FV, 0.0f, DVI400_2V, DVI400_400UA, RELAY_ON);
	vin.Set(FV, 5.5f, PVI10A_10V, PVI10A_1MA, RELAY_ON);
...
DUT_API int Vout(short funcindex, LPCTSTR funclabel)
{
	...
	///////////////////////// Vout1 /////////////////////////
	vin.Set(FV, 3.4f, PVI10A_10V, PVI10A_1A,RELAY_ON);
	en.Set(FV, 1.2f, DVI400_5V, DVI400_400UA, RELAY_ON);
	delay_ms(2);
	vout.Set(FI, -5.0e-3, PVI10A_5V, PVI10A_1A,RELAY_ON);
	delay_ms(5);
	vout.Set(FI, -10.0e-3, PVI10A_5V, PVI10A_1A,RELAY_ON);
	delay_ms(1);

	vout.MeasureVI(MV, 100, 20);
	for (i=0;i<SITENUM;i++)
	{
	...
	for (i=0;i<SITENUM;i++)
	{
		v3[i]=vout.GetMeasResult(i)-0.000f;
	...
	vout.Set(FV, 0.0f, PVI10A_5V, PVI10A_1A,RELAY_ON);
	en.Set(FV, 0.0f, DVI400_10V, DVI400_400UA, RELAY_ON);
...
DUT_API int Ven(short funcindex, LPCTSTR funclabel)
{
	...
	en.Set(FV, 0.9f, DVI400_5V, DVI400_400UA, RELAY_ON);
	vout.Set(FI, -0.5e-3f, PVI10A_5V, PVI10A_100MA,RELAY_ON);
	delay_ms(1);
	//////////// Ven_h ////////////
	for(i=0; i<SITENUM; i++)
	{
	...
	for(k=0.9f; k<1.4f; k=k+0.01f )  
	{
		...
		{ 
			...
			if(adresult[i+4]>300.0e-6f && check[i]<0.5f)
			{
				adresult[i+8]=k;
				check[i]=1.0f;
			}
	...
	vin.Set(FV, 5.5f, PVI10A_10V, PVI10A_100MA,RELAY_ON);
	en.Set(FV, 1.4f, DVI400_5V, DVI400_400UA, RELAY_ON);
	vout.Set(FI, -0.5e-3f, PVI10A_5V, PVI10A_100MA,RELAY_ON);
	delay_ms(1);
	...
	for(k=1.2f; k>0.85f; k=k-0.01f )  
	{
		...
		delay_us(200);
		vin.MeasureVI(MI,10,200);//100,20
		for(i=0; i<SITENUM; i++ )  
		{ 
			adresult[i+12]=vin.GetMeasResult(i);
			//if(adresult[i+12]<0.00005f && check[i]<0.5f)
			////if(adresult[i+12]<100.0e-6f && check[i]<0.5f)
			if(adresult[i+12]<200.0e-6f && check[i]<0.5f)
			{
				adresult[i+16]=k;
				check[i]=1.0f;
			}
...
DUT_API int UVLO(short funcindex, LPCTSTR funclabel)
{
	...
	vout.Set(FI, -0.5e-3, PVI10A_5V, PVI10A_100MA,RELAY_ON);
	delay_ms(1);
	//////////// UVLO_R ////////////
	for(i=0; i<SITENUM; i++)
	{
	...
	for(k=1.8f; k<2.5f; k=k+0.01f )  
	{
		...
		{ 
			adresult[i+4]=vin.GetMeasResult(i);
			//if(adresult[i+4]>0.0009f && check[i]<0.5f)
			if(adresult[i+4]>400.0e-6f && check[i]<0.5f)
			{
				adresult[i+8]=k;
				check[i]=1.0f;
			}
	...
	vin.Set(FV, 2.4f, PVI10A_10V, PVI10A_100MA,RELAY_ON);
	//delay_ms(10);
	...
	for (k=2.4f; k>1.7f; k= k-0.01f)
	{
		...
		delay_us(100);
		vin.MeasureVI(MI,100,20);
		for(i=0; i<SITENUM; i++ )  
		{ 
			adresult[i+12]=vin.GetMeasResult(i);
			//if(adresult[i+12]<0.00005f && check[i]<0.5f)
			//if(adresult[i+12]<80.0e-6f && check[i]<0.5f)
			if(adresult[i+12]<20.0e-6f && check[i]<0.5f)
			{
				adresult[i+16]=k;
				check[i]=1.0f;
			}
...
DUT_API int OVP(short funcindex, LPCTSTR funclabel)
{
	...
	for (k=6.5f; k>5.5f; k=k-0.01f)
	{
		...
		delay_us(200);
		vin.MeasureVI(MI,10,200);// 10,20
		for(i=0; i<SITENUM; i++ )  
		{ 
			adresult[i+12]=vin.GetMeasResult(i);
      if(adresult[i+12]>90.0e-6f && check[i]<0.5f)
		  ////if(adresult[i+12]>500.0e-6f && check[i]<0.5f)
            ////if(adresult[i+12]>90.0e-6f && check[i]<0.5f)
			{
				adresult[i+16]=k;
				check[i]=1.0f;
...
DUT_API int Dropout(short funcindex, LPCTSTR funclabel)
{
	...
	////vin.Set(FV, 3.6f, PVI10A_10V, PVI10A_1A, RELAY_ON);
	vin.Set(FV, 1.5f, PVI10A_10V, PVI10A_1A, RELAY_ON);
	...
	for ( i=0; i<SITENUM; i++)
	{
		////for (vx=3.6f; vx>(2.8f*0.9f); vx=vx-0.01f)
		for (vx=1.5f; vx>(0.9f*0.9f); vx=vx-0.01f)
	    {
			vin.Set(FV, vx, PVI10A_10V, PVI10A_1A, RELAY_ON);
			delay_us(100);
			vout.MeasureVI(MV,10,10);
			adresult[i]=vout.GetMeasResult(i);
			if ((flag[i] == 0) && (adresult[i] < v1[i]*0.98f)) //adresult[]<vout*95%则不扫描，根据扫描标志状态决定是否进行扫描判断
            {
					result[i] = vx;
